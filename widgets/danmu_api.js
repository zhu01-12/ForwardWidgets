...
WidgetMetadata = {
  ...
  description: "从接口中获取弹幕，支持屏蔽关键词过滤",
  author: "，"小振
...
// 解析屏蔽关键词
function parseBlockKeywords(keywordsStr) {
...
// 检查是否包含屏蔽关键词
function containsBlockKeywords(text, blockKeywords) {
  if (!text || !blockKeywords || blockKeywords.length === 0) {
    return false;
  }
  
...
// 过滤含有屏蔽关键词的动漫
function filterAnimesByBlockKeywords(animes, blockKeywords) {
 ...
  
  return animes.filter(anime => {
    // 检查动漫标题
    if (anime.animeTitle && containsBlockKeywords(anime.animeTitle, blockKeywords)) {
      return false;
    }
    
    // 检查动漫类型（如果有的话）
    if (anime.type && containsBlockKeywords(anime.type, blockKeywords)) {
      return false;
    }
    
    // 检查动漫描述（如果有的话）
    if (anime.description && containsBlockKeywords(anime.description, blockKeywords)) {
      return false;
    }
    
    return true;
...
async function searchDanmu(params) {
 ...
  
  // 解析屏蔽关键词
  const blockKeywords = parseBlockKeywords(params.blockKeywords);
  ...
  if (blockKeywords.length > 0) {
    ...
    animes = filterAnimesByBlockKeywords(animes, blockKeywords);
    console.log(`屏蔽关键词过滤：${originalCount} -> ${animes.length}（过滤了${originalCount - animes.length}个结果）`);
  }
